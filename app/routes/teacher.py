...
def achievement_submit_upload():
    """处理文件上传（复用管理员逻辑，但submitter_type为teacher）"""
    import hashlib
    ...
    try:
        ...
        if lab_association_mode and lab_association_mode != 'none':
            if lab_association_mode.startswith('specific:'):
                try:
                    ...
                    pass
            # auto 模式在更新 achievement_data 后根据第一导师解析
                    ...
                    try:
                        ...
                        if preview_path:
                            ...
                            try:
                                ...
                                logger.warning("[教师上传PDF预览] 预览图不在 files_root 下: %s", preview_path)
                    except Exception as e:
                        logger.warning("[教师上传PDF预览] 生成失败: %s", e, exc_info=True)
        ...
        # 跳转到文件导入结果页面（教师版本）
        redirect_url = url_for('teacher.achievement_submit_results', session_id=import_session_id)
