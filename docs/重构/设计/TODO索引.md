...
| T75 | **冻结模块处置·方案 C 已落地（2026-08-23）**：4 轻伤文件解冻转正（122 例正常执行，14 例分诊详见《2026-08-23_T75方案C分诊台账》，13 例定性「期望过时」修断言、1 例 parse_students 全角括号疑真缺陷单例 xfail 挂账）；**剩余=certificate_extractor 18 例解冻分诊待 hewj 定义现行契约后继续**（实测其中 6 例现行 XPASS 大概率免修）。CI 已收编 4 文件 | 决策分析 P2 批3 / 方案C goal | 🟡 剩余挂账 |
