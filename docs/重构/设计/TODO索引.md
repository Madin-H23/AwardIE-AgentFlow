...
| ~~T49~~ | ~~三模式**生成期真流式**~~ **已完成（2026-08-23，T49 goal）**：tools=AgentService.chat_stream（create_agent messages 流+AIMessageChunk 过滤）；auto=run_stream 三通道单次订阅 [updates,messages,values]（qa 白名单 delta+progress 保留+values 终态消灭双跑）；workflow/service build_chat_model 开 streaming=True；多轮语义定案 done.output≡delta 拼接；护栏测试 test_t49_true_streaming 5 例；playwright 实测 qa40/auto56/tools738 delta 全生成期+拼接一致 | T49 goal / 实施记录《2026-08-23_T49三模式生成期真流式》 | ✅ | 可选→完成 |
...
| ~~T74~~ | ~~旧基类 17 页分批迁 base_console~~ **已完成（2026-08-23，决策落档型关闭）**：实测 16 页 user_base 并非遗留债务，而是**师生门户双壳体系的门户基类**——T45 双壳定案（admin=console / 师生=portal）晚于 T9，为现行有效设计。分类：`*_ref` 假数据参考页×6（在册渲染正常）＋师生功能页×10（profile/submissions×2/submissions_list×2/data_export/results_portal/chat_portal）。迁入 admin console 壳将令师生见到管理员导航＝体验倒退，故**保留不迁**。**base_simple 登录页保留决策**：登录前无角色、管理侧边栏无意义、vendor 本地化后无外链风险，保留轻量独立壳。**重估触发**：若 hewj 拍板师生端统一控制台体系，随 T9 流水线整体迁移（届时本行重开） | 出处同前 | ✅决策落档 |
| ~~T75~~ | ~~冻结模块处置~~ **已完成·全清账（2026-08-24，方案 C + 契约签认收尾）**：4 轻伤文件解冻转正收编 CI（122 例）；hewj 签认《2026-08-24_certificate_extractor契约文档》后 certificate 12 例构造修复+18 例全部常规执行+CI 收编第 5 文件——**140 例冻结测试全部恢复常驻防线**。parse_students 双缺陷（吞全角括号+P3 分组错位）已经 hewj 裁决授权于 2026-08-24 解冻单点修复完毕（见台账 §六），T75 无任何剩余 | 决策分析 P2 批3 / 方案C goal / 收尾 goal | ✅ 全清账 |
