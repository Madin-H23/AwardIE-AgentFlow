...
| # | 事项 | 依赖 | 产出 | 预计用例 | 触发/时点 |
| --- | --- | --- | --- | --- | --- |
| T38 | L1 数据层：迁移 0006 + SystemEventLog ORM 模型 + SystemEventLogger（含 PII 脱敏、不阻塞）+ 接入点矩阵（7 类事件，即 T27） | 无 | migrations/0006 + backend/orm/system_event_log.py + backend/utils/system_event_logger.py | ~12 | 高 |
| T39 | L2 查询层：LogQueryService（四源统一查询）+ LogFileReader + MetricsSnapshot + trace_id 补齐（即 T26：TraceIdFilter + before_request + 解析正则） | L1 | backend/services/log_query_service.py 等 3 个 | ~15 | 高 |
