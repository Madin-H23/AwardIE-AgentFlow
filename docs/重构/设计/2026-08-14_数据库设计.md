...
| --- | --- |
| **文档版本** | v1.4 |
| **编写日期** | 2026-08-17（v1.4 2026-08-19：§7 实施状态回写） |
...
> **2026-08-19 实施完成状态（v1.4 回写，M1 后半全部收官）**：Alembic 迁移链已在真实库落成并应用（备份可回退）——
> | 迁移 | 内容 | 实际落地 |
> | --- | --- | --- |
> | `0001_orm_baseline` | no-op baseline（现库即起点），stamp | ✅ `3f5bf82` |
> | `0002_legacy_tables_to_views` | 5 张关联表 student_id 重写为 users.id（257 行零悬空）+ FK 改指 `users(id)`；旧三表 → 视图指向 users（`id=users.id`、`student_id←login_code`） | ✅ `18d694e` + 写路径全收口 `d677a44` |
> | `0003_typization_rebuild` | 18 表短字段 TEXT→VARCHAR(N)（映射表 §2）；保留 is_valid 生成列 / 42 索引 / 三视图 / FK | ✅ `2174336` + 双写收尾 `4667999` |
>
> **相对 §7 规划的偏差（诚实标注）**：原规划"P2 收尾观察一个迭代后 DROP 旧三表"——实际实施为**直接视图化**（users 已成为唯一写真源，旧表无可写路径后立即转视图），未再等待观察期；`users_sync` 写桥（sync_user_row/insert_user_row）已退役删除，仅留 `to_users_id` 桥接。备份：`database/bak.2026-08-19-refrewrite.db` / `bak.2026-08-19-orm-migrate.db` / `bak.2026-08-19-typization.db`。
> 实施细节与验证（往返测试、GUI 冒烟）见《2026-08-19_阶段五-二期深化实施文档.md》§2.6-2.8。
