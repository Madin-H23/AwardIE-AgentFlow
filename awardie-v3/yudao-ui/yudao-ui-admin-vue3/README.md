...
| 项 | 值 |
|---|---|
| 上游仓库 | `yudaocode/yudao-ui-admin-vue3` |
| 同步 commit | **`aab14fb0`** |
| 许可证 | MIT(见 `LICENSE`,Copyright Archer) |
| 策略 | **框架冻结,不做例行上游升级**(决策见 `docs/adr/0004-前端框架冻结与分层所有权.md`) |

### 三层所有权

框架层冻结的判准是「批11 就是切流,切流前不应有例行上游同步」。冻结**不等于框架不能动**——
遇到框架 bug 仍需能改,**改了就算脱离上游**,这是冻结的代价。安全补丁走锁定 `pnpm-lock.yaml`
定向升级依赖,不整体拉上游。

| 目录 | 归属 | 说明 |
|---|---|---|
| `src/layout` `src/components` `src/config` `src/store` `src/router` | 框架层 | 芋道原样,主动不改 |
| `src/views/{infra,system}` | 框架层 | 119 个系统管理页 |
| `src/api/business/**` | 业务层 | 我们自己写,按 v3 后端 API |
| `src/views/business/**` `src/views/portal/**` | 业务层 | 九管理页 + 教师页 + 学生门户 |
| `src/views/business/components/**` | 交互层 | 从 v2 移植的 UX-1 资产,自定义组件 |

### 相对上游的改动

**删除的示例业务模块(24 个)**:ai bpm crm erp fms hrm im iot mall member mes mp pay pms report wms oa
——只留 `infra` / `system` 两套系统管理能力。`src/views` 25 目录→8,`src/api` 20→3。

**连带删除的死代码**(删模块后仅靠构建查不出,`ts:check` 才暴露):
`components/bpmnProcessDesigner`(67 文件)、`components/SimpleProcessDesignerV2`(37 文件)、
`store/modules/bpm`、`store/modules/mall` —— 均只互相引用、无人引用,且依赖已删的 `@/api/bpm/*`、`@/api/mall/*`。

**清掉的悬空引用**:
- `router/modules/remaining.ts` 删业务路由段并**补回被截断的 `export default remainingRouter`**
- `main.ts` 移除 BPM 的 wangEditor 插件
- `layout/components/ToolHeader.vue` 移除财务科目切换器
- `components/AppLinkInput/AppLinkSelectDialog.vue` 商城商品分类选择器 → 通用 `el-input-number` + 补确认按钮
- `components/DiyEditor/components/mobile/*` 删 7 个商城卡片(6 个营销 + `PromotionArticle`);
  `DiyEditor/util.ts` 的 `PAGE_LIBS` 同步移除已空的两个分组

**必须入库的生成物**:`src/types/auto-imports.d.ts` 与 `auto-components.d.ts` 由 unplugin-auto-import
在 **dev 模式**生成(`dts: !isBuild`),CI 只跑 `build` 生成不出来。上游 `.gitignore` 的 `auto-*.d.ts`
已改为只忽略仓库根目录的同名文件,否则 `ts:check` 会报满屏 `Cannot find name 'ref'`。

### 上游 HEAD 自身构建不过

`aab14fb0` 的 `src/views/oa/attendance/*` 三个文件 import `@/views/oa/utils/constants`,
**该目录在上游仓库里不存在**。我们靠删掉 OA 模块绕过。下次若解冻并升级上游,**先查这一条**,
别以为是自己的问题。
...
```bash
pnpm install
pnpm dev              # 开发(顺带生成 auto-imports.d.ts)
pnpm build:local      # 硬门禁
pnpm ts:check         # 类型检查,依赖已入库的 auto-*.d.ts
pnpm lint:eslint      # eslint --fix
```

`node_modules` 与 `dist` 不入库。后端在 `http://localhost:48080`,`VITE_API_URL=/admin-api` 由
`.env.local` 提供,API 层只写 `/business/...`。

---

以下是上游(芋道)原始 README,仅作框架用法参考。
