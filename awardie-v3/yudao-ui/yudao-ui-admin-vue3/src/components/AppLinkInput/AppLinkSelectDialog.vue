<template>
  ...
  <Dialog v-model="detailSelectDialog.visible" title="" width="50%">
    <el-form class="min-h-200px">
      ...
      >
        <!-- AwardIE 无商品中心,这里不挂具体业务选择器,改为手填 id -->
        <el-input-number v-model="detailSelectDialog.id" :min="1" controls-position="right" />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="handleDetailIdConfirm">确定</el-button>
      </el-form-item>
    </el-form>
  </Dialog>
</template>
...
// 确认 id 写入链接
const handleDetailIdConfirm = () => {
  const id = detailSelectDialog.value.id
  if (id) {
