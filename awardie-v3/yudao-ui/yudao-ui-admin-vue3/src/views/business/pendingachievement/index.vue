<template>
      ...
      <el-table-column label="操作" width="200" align="center" fixed="right">
        <template #default="scope">
          ...
          <template v-if="scope.row.status === 'pending'">
            <el-button
              v-hasPermi="['business:pending-achievement:review']"
              link
              type="success"
              @click.stop="openReview(scope.row, 'approve')"
            >
              通过
            </el-button>
            <el-button
              ...
              link
              type="danger"
              @click.stop="openReview(scope.row, 'reject')"
            >
              驳回
            </el-button>
          </template>
  ...
  <el-drawer v-model="detailVisible" title="待审成果详情" size="720px">
    <div v-if="detail" v-loading="detailLoading" class="detail-body">
      <el-descriptions :column="2" border size="small">
        ...
        <el-descriptions-item label="校验结果">{{ validationLabel(detail.validationResult) }}</el-descriptions-item>
      ...
      <el-button v-if="fileUrl" size="small" @click="handleDownload">
        <Icon icon="ep:download" class="mr-5px" />下载原文件
      </el-button>
      ...
      <div v-loading="aiLoading">
        <template v-if="aiSuggest">
          <el-alert
            ...
            :type="aiSuggest.decision === 'pass' ? 'success' : aiSuggest.decision === 'reject' ? 'error' : 'warning'"
            :closable="false"
          />
      ...
      <el-timeline v-if="timeline.length">
        ...
        >
          ...
          <div v-if="item.changeDetail" class="text-12px text-gray-500">{{ item.changeDetail }}</div>
        </el-timeline-item>
      </el-timeline>
  ...
  <Dialog v-model="reviewVisible" :title="reviewAction === 'approve' ? '通过确认' : '驳回确认'" width="520px">
    <el-alert
      ...
      type="warning"
      :closable="false"
      title="通过后会把这条成果物化写入成果库(奖状类写入 awardie_awards),此操作不可撤销"
      class="mb-12px"
    />
...
defineOptions({ name: 'PendingAchievement' })
...
// 留痕动作码与后端一致:1 提交 / 6 通过 / 7 驳回 / 8 物化
...
/**
 * teacher-pending-list 是全量 List(无分页、无类型筛选),类型筛选与分页在前端做。
 * 成果类型筛选用前端,是因为该端点只接受 status 一个参数。
 */
...
/** achievementData 是 JSON 字符串,不是嵌套对象 */
...
const openDetail = async (row: any) => {
  ...
  try {
    const [tl, ai] = await Promise.allSettled([
      ...
      // 非 pending 状态后端会拒(4009),已经审过的行没必要再问 AI
...
const handleDownload = async () => {
  const data = await downloadPending(detail.value.id)
  download.excel(data, `待审成果-${detail.value.id}`)
}
...
const submitReview = async () => {
  ...
  try {
    ...
    await getList()
    // 抽屉开着就同步刷新详情,避免审完还显示旧状态
    if (detailVisible.value && detail.value) {
      const fresh = allList.value.find((r) => r.id === detail.value.id)
      if (fresh) {
        detail.value = fresh
      }
    }
