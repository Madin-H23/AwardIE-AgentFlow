...
class AwardIntegrationTest:
    ...
    def __init__(self):
        ...
        # 测试文件目录
        self.test_images_dir = self.project_root / "tests" / "test_images" / "award"
        self.chinese_dir = self.test_images_dir / "chinese"
        self.english_dir = self.test_images_dir / "english"
        self.other_dir = self.project_root / "tests" / "test_images" / "other"
        ...
        # 初始化模板管理器
        db_path = str(self.config_loader.get_path("database", "competitions_db"))
        extract_config = config.get("extract", {})
        base_fields_map = {}

        # 加载各类型的字段定义
        for doc_type in ["award", "patent", "software"]:
            fields_file = self.project_root / "backend" / "extract" / "prompts" / f"{doc_type}_fields.json"
            if fields_file.exists():
                import json
                with open(fields_file, "r", encoding="utf-8") as f:
                    base_fields_map[doc_type] = json.load(f)

        # config_dir should point to backend/extract/config (where type_rules.json is located)
        config_dir = self.project_root / "backend" / "extract" / "config"
    ...
    def run_all_tests(self):
        ...
        if not self.extractor_available:
            print("\n⚠ AwardExtractor 尚未实现，跳过测试")
            return

        # 1. 测试抽取器初始化
        self.results.append(self._test_extractor_init())

        # 2. 测试中文奖状抽取
        if self.chinese_dir.exists():
            chinese_files = list(self.chinese_dir.glob("*"))
            if chinese_files:
                print(f"\n找到 {len(chinese_files)} 个中文奖状文件")
                for file_path in chinese_files:
                    self.results.append(self._test_award_extraction(file_path, "chinese"))
            else:
                print("\n未找到中文奖状文件")
        else:
            print(f"\n中文奖状目录不存在: {self.chinese_dir}")

        # 3. 测试英文奖状抽取
        if self.english_dir.exists():
            english_files = list(self.english_dir.glob("*"))
            if english_files:
                print(f"\n找到 {len(english_files)} 个英文奖状文件")
                for file_path in english_files:
                    self.results.append(self._test_award_extraction(file_path, "english"))
            else:
                print("\n未找到英文奖状文件")
        else:
            print(f"\n英文奖状目录不存在: {self.english_dir}")

        # 4. 测试其他文件（应返回other）
        if self.other_dir.exists():
            other_files = [f for f in self.other_dir.glob("*") if f.is_file()]
            # 只测试几个
            for file_path in other_files[:3]:
                self.results.append(self._test_other_file(file_path))

    def _test_extractor_init(self) -> TestResult:
        """测试抽取器初始化"""
        name = "抽取器初始化"
        start_time = time.time()

        try:
            config = self.config_loader.load_config().get("extract", {}).get("award", {})
            extractor = self.AwardExtractor(config, template_manager=self.template_manager)
            duration = time.time() - start_time

            if extractor.name == "award" and extractor.template_type == "award":
                return TestResult(
                    name=name,
                    passed=True,
                    message="抽取器初始化成功",
                    duration=duration,
                    expected="name=award, template_type=award",
                    actual=f"name={extractor.name}, template_type={extractor.template_type}"
                )
            else:
                return TestResult(
                    name=name,
            ...
            return TestResult(
                name=name,
                passed=False,
                message=f"抽取器初始化失败: {e}",
                duration=time.time() - start_time
            )

    def _test_award_extraction(self, file_path: Path, award_type: str) -> TestResult:
        """测试奖状抽取"""
        name = f"{'中文' if award_type == 'chinese' else '英文'}奖状抽取 - {file_path.name}"
        start_time = time.time()
        ...
        try:
            ...
            if result.template_type == "award":
                ...
                # 检查关键字段
                required_fields = ["competition_name", "award_level"]
                ...
                # 检查获奖者字段
                has_winner = data.get("winners") or data.get("winner_name")
                if not has_winner:
                    return TestResult(
                        name=name,
                        passed=False,
                        message="缺少获奖者信息 (winners 或 winner_name)",
                        duration=duration,
                        details={"data": data}
                    )
                ...
                return TestResult(
                    name=name,
                    passed=True,
                    message=f"成功抽取: {data.get('competition_name', '')} - {data.get('award_level', '')}",
                    duration=duration,
                    details={
                        "data": data,
                        "ocr_cache_hit": result.metadata.get("ocr_cache_hit", False) if result.metadata else False,
                        "llm_cache_hit": result.metadata.get("llm_cache_hit", False) if result.metadata else False
                    }
                )
                ...
                return TestResult(
                    name=name,
                    passed=False,
                    message=f"未识别为奖状: {result.data.get('note', result.error_message or '')}",
                    duration=duration
                )
            else:
                return TestResult(
    ...
    def _test_other_file(self, file_path: Path) -> TestResult:
        """测试其他文件（应返回other）"""
        name = f"其他文件处理 - {file_path.name}"
        start_time = time.time()
        ...
        try:
            ...
            if not extractor.matches_extension(ext):
                return TestResult(
                    name=name,
                    passed=True,
                    message=f"正确跳过不支持的扩展名: {ext}",
                    duration=time.time() - start_time
                )
            ...
            if result.template_type == "other":
                return TestResult(
                    name=name,
                    passed=True,
                    message=f"正确识别为非奖状文件",
                    duration=duration,
                    details={"note": result.data.get("note", "")}
                )
            else:
                return TestResult(
                    name=name,
...
def main():
    ...
    report_path = report_dir / "奖状抽取器集成测试报告.html"
