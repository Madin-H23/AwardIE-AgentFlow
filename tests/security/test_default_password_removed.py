...
def tmp_users_db(tmp_path):
    ...
    conn.execute("INSERT INTO students VALUES ('s1','正常学生',?, 'student', 1, 0)", (generate_password_hash('GoodPass123'),))
    conn.execute("INSERT INTO students VALUES ('s2','空hash学生', NULL, 'student', 1, 0)")
    conn.execute("INSERT INTO teachers VALUES ('t1','空hash教师', NULL, 'teacher', 1, 0)")
    conn.execute("INSERT INTO teachers VALUES ('t2','未激活教师', ?, 'teacher', 0, 0)", (generate_password_hash('GoodPass123'),))
    conn.commit()
    conn.close()
    return str(db)
