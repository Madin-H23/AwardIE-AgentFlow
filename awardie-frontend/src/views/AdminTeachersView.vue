...
// #29/#38 对照 v1 admin/teachers/list.html:页头+添加/搜索 c-panel/表格(工号/姓名/部门/职称/电话/激活)+编辑/删除。
...
const form = reactive({
  loginCode: '', name: '', department: '', title: '', phone: '',
})
...
function openCreate() {
  ...
  Object.assign(form, { loginCode: '', name: '', department: '', title: '', phone: '' })
...
function openEdit(row: Row) {
  ...
  Object.assign(form, {
    loginCode: row.login_code, name: row.name, department: row.department ?? '',
...
async function save() {
  ...
  const body = editingId.value === null
    ? await apiJson('POST', '/api/v2/admin/users/teacher', { ...form })
    : await apiJson('PUT', `/api/v2/admin/users/teacher/${editingId.value}`, { ...form })
  if (body.code === 0) {
...
async function remove(row: Row) {
  const ok = await ElMessageBox.confirm(
    `确定删除教师 ${row.name}(${row.login_code})吗?存在关联指导数据时将拒绝。`, '删除确认', { type: 'warning' },
  ).catch(() => false)
  if (!ok) return
  const body = await apiJson('DELETE', `/api/v2/admin/users/teacher/${row.id}`)
  if (body.code === 0) {
...
<template>
  <div>
    ...
    <el-dialog
      ...
      :title="editingId === null ? '添加教师' : '编辑教师'"
      width="460px"
    >
